...
const getToken = async (code) => {
  ...
  const response = await fetch(
    'https://bjw5g7t12b.execute-api.us-west-1.amazonaws.com/dev/api/token' + '/' + encodeCode
  );
...
export const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url = "https://bjw5g7t12b.execute-api.us-west-1.amazonaws.com/dev/api/get-events" + "/" + token;
...
export const getAccessToken = async () => {
  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const response = await fetch(
        "https://bjw5g7t12b.execute-api.us-west-1.amazonaws.com/dev/api/get-auth-url"
      );
      const result = await response.json();
